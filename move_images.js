...
var fromPath = '/Users/admin/Documents/GitHub/Avias/Resources/iphone/images';
var toPathHigh = '/Users/admin/Documents/GitHub/Avias/Resources/android/images/high';
var toPathMiddle = '/Users/admin/Documents/GitHub/Avias/Resources/android/images/medium'

readEachFileMatching(/\.png$/, fromPath, function(err, file, stat, content) {
    var fs = require('fs-extra');
    var ensureDir = require('ensureDir');

    if (file.search('@2x') != -1) {
        var fp = file.replace(fromPath, toPathHigh).replace("@2x", "");
        var fpArr = fp.split('/');
        var dp = fp.replace('/' + fpArr[fpArr.length - 1], '');
        ensureDir(dp);

        fs.copy(file, fp, function(err) {
            if (err) {
                console.error(err);
            } else {
                console.log("success!")
        ...
        var fp = file.replace(fromPath, toPathMiddle);
        var fpArr = fp.split('/');
        var dp = fp.replace('/' + fpArr[fpArr.length - 1], '');
        ensureDir(dp);
